...
function BodyReader(options) {
    ...
    function readFldChar(element) {
        ...
        if (type === "begin") {
            ...
            if (inField) {
              inField = false;
              return elementResult(new documents.Text('{{{/span}}}'));
            }
    ...
    function readInstrText(element) {
        ...
        if (inField && json && json.match(/ *ADDIN ZOTERO_ITEM CSL_CITATION */)) {
            json = json.replace(/ *ADDIN ZOTERO_ITEM CSL_CITATION */, "");
            var obj = JSON.parse(json);
            /*
             * To finish ...
             * - Extract key from URI
             * - Convert items to CSL-M JSON
             * - Write items files
             * - Build citation ID from doc position and citationItems key, prefix, suffix, and locator sets
             * - Notes:
             *   1. document-stored metadata is CSL-M, not portable CSL, so can be used directly (a happy accident)
             *   2. verbose value of jurisdiction must be recreated, use juris-desc records for this purpose.
             *   3. the HTML output will need to be post-processed to convert {{{span}}}...{{{/span}}} to <span>...</span>
             */
            return elementResult(new documents.Text('{{{span}}}'));
        }
        return emptyResult();
